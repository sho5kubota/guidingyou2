...
class ProductCore extends ObjectModel
{
	...
	/** @var string Object latitude */
	public $latitude;

	/** @var string Object longitude */
	public $longitude;
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'latitude' => 					array('type' => self::TYPE_STRING, 'validate' => 'isReference'),
			'longitude' => 					array('type' => self::TYPE_STRING, 'validate' => 'isReference'),
			...
			'country' => 					array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'),
			'state' => 						array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'),
			'address' => 					array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml', 'required' => true),
			'address2' => 					array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isCleanHtml'),
			'city' => 						array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'),
	...
	public static function getProductLikeName($id_lang = null, $name, $field) {

		$sql = '
		SELECT `id_product`
		FROM `'._DB_PREFIX_.'product_lang`
		WHERE `' . $field . '` LIKE \''.pSQL($name).'%\'';
		if ($id_lang)
			$sql .= ' AND `id_lang` = '.(int)$id_lang;

		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql);

		return (boolean)!empty($result['id_product'])  ? 1 : 0;
	
	}

	public static function getProductStateLikeName($name) {
		...
		return (boolean)!empty($result['id_state'])  ? 1 : 0;
