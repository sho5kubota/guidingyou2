...
class FacebookPsConnect extends Module
{
	...
	public function displayModule($sTplName, $aAssign)
	{
		if (file_exists(_FPC_PATH_TPL . $sTplName) && is_file(_FPC_PATH_TPL . $sTplName)) {
			...
			$aAssign = array_merge($aAssign, array('sModuleName' => strtolower(_FPC_MODULE_NAME), 'bDebug' => _FPC_DEBUG, 'iCompare' => version_compare(_PS_VERSION_, '1.4.1')));
	...
	public function displayErrorModule()
	{
		...
		$smarty->assign(
			array(
				...
				'sModuleName'   => strtolower(_FPC_MODULE_NAME),
				'bDebug'        => _FPC_DEBUG,
			)
		);

		return (
			$this->display(__FILE__, _FPC_PATH_TPL_NAME . _FPC_TPL_ERROR)
		);
	}
