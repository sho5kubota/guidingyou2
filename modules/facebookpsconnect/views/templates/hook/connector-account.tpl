	...
	<div id="ao_fpsc_fancybox" style="display:none; visibility:hidden;">
		<div id="socialMessage">
			...
			<div id="fpcFancyboxContent">
				...
				<p id="connectorText">
					{if $bFriendlyPermission == true}
						{l s='Link your PrestaShop account to your Facebook profile NOW, using the Facebook Connect button below !' mod='facebookpsconnect'}
						<br />
						{l s='By doing so, we will also be able to send you personalized offers and promotions based on your Facebook profile and Likes. In addition, by sharing your friend list and a small portion of their information, we will also be able to send you gift ideas for their birthdays and other special occasions.' mod='facebookpsconnect'}
					{else}
						{l s='Link your PrestaShop account to your Facebook profile NOW, using the Facebook Connect button below !' mod='facebookpsconnect'}
						<br />
						{l s='By doing so, we will also be able to send you personalized offers and promotions based on your Facebook profile and Likes.' mod='facebookpsconnect'}
					{/if}
				</p>
			</div>
			...
			<p align="right"><a onclick="{$sModuleName|escape:"html"}.ajax('{$sModuleURI|escape:"html"}','sAction=update&sType=customer&id={$iCustomerId|escape:"html"}','ajax_customer_response');$.fancybox.close();" href="#">{l s='Do not show this message again' mod='facebookpsconnect'}</a></p>
			...
			<div class="ao_fpsc_clr_hr"></div>
			<div class="ao_fpsc_clr_20"></div>
			<form action="" class="form-horizontal" method="post" id="{$sModuleName|escape:"html"}TwitterForm" name=''>
				<input type="hidden" name="sAction" value="update" />
				<input type="hidden" name="sType" value="email" />
				<input type="hidden" name="connector" value="{$sConnector|escape:"html"}" />
				<input type="hidden" name="customerId" value="{$iCustomerId|escape:"html"}" />
				<div class="bootstrap">
					...
					<center>
						<button id="btn-save" name="{$sModuleName|escape:"html"}Button" class="button btn btn-success" value="{l s='Send' mod='facebookpsconnect'}"  onclick="{$sModuleName|escape:"html"}.form('{$sModuleName|escape:"html"}TwitterForm', '{$sModuleURI|escape:"html"}', '', 'socialMessage', 'socialMessage', false, false, null, 'Email');return false;" >{l s='Send' mod='facebookpsconnect'}</button>
					</center>
				</div>
			</form>
			<div id="{$sModuleName|escape:"html"}EmailError"></div>
		</div>
		{/if}
	</div>
	</div>
