	...
	{$post.short_description|html_entity_decode|strip_tags}</div></span>
        ...
        <div class="sdsreadMore">
                  {assign var="options" value=null}
                        ...
                        <a href="{$link->getAgileSellerLink($post.id_author)}" style="float:right">{l s="Go to this guide page" mod="smartblog"}</a> 
                         <a title="{$post.meta_title}" href="{smartblog::GetSmartBlogLink('smartblog_post',$options)}" class="r_more">{l s='Read more' mod='smartblog'} </a></span>
        </div>
   </div>
</div>