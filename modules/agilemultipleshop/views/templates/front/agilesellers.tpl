...
<script language="javascript" type="text/javascript">
	$(document).ready(function () {
		...
		$('div.filters select#seller_location').change(function() {
			jumptoagilesellerspage("{$base_dir_ssl}","{$filter}", "{$loclevel}", {$parentid}, 0);
		});

		$('div.filters select#seller_location_state').change(function() {
			jumptoagilesellerspage("{$base_dir_ssl}","{$filter}", "{$loclevel}", {$parentid}, 1);
		});


	});
</script>
<h2>{l s='Guide List' mod='agilemultipleshop'}</h2>
<br>

<!-- <pre>{$states|@print_r}</pre> -->
<div class="filters">
	<div class="row">
		...
		<div class="agile-col-xs-12 agile-col-sm-6 agile-col-md-4 clearfix">
			<label class="agile-col-xs-4 agile-col-sm-4 agile-col-md-4 agile-col-lg-3 agile-col-xl-3">{l s='Country:' mod='agilemultipleshop'}</label>
 			<select name="seller_location" id="seller_location" class="agile-col-xs-8 agile-col-sm-8 agile-col-md-8 agile-col-lg-6 agile-col-xl-8">
				<option value="">{l s=' - All Locations-' mod='agilemultipleshop'}</option>
				{foreach from=$countries item=sellerloc}
					{if !empty({$sellerloc.name})}
					<option value="{$sellerloc.id}" {if $sellerloc.id == $parentid}selected{/if}>
						{$sellerloc.name}
					</option>
					{/if}
				{/foreach}
		...
		{if count($states) > 0}
			<div class="agile-col-xs-12 agile-col-sm-6 agile-col-md-4 clearfix">
			<label class="agile-col-xs-4 agile-col-sm-4 agile-col-md-4 agile-col-lg-3 agile-col-xl-3">{l s='State:' mod='agilemultipleshop'}</label>
 			<select name="seller_location_state" id="seller_location_state" class="agile-col-xs-8 agile-col-sm-8 agile-col-md-8 agile-col-lg-6 agile-col-xl-8">
				<option value="">{l s=' - All Locations-' mod='agilemultipleshop'}</option>
				{foreach from=$states item=sellerloc}
					{if !empty({$sellerloc.name})}
					<option value="{$sellerloc.id}" {if $sellerloc.id == $seller_location}selected{/if}>
						{$sellerloc.name}
					</option>
					{/if}
				{/foreach}
		...
		<div class="block_content">
			...
			{foreach from=$seller_list item=seller name=gridViewSellers}
				...
				<div class="seller-container">
					...
					<img src="{$seller.seller_logo_url}" height="250" style="max-width: 250px" alt="{$seller.company|escape:html:'UTF-8'}"  />
					{* <img src="{$seller.seller_logo_url}" height="{$homeSize.height}" width="{$homeSize.width}" alt="{$seller.company|escape:html:'UTF-8'}"  /> *}
					</a>
					</div>
