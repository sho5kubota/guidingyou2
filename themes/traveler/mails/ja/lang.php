<?php

global $_LANGMAIL;
$_LANGMAIL = array();
$_LANGMAIL['Welcome!'] = 'Guiding Youの会員登録情報';
$_LANGMAIL['Process the payment of your order'] = '';
$_LANGMAIL['Message from contact form'] = 'ゲストからのメッセージ';
$_LANGMAIL['Your message has been correctly sent #ct%1$s #tc%2$s'] = 'メッセージは正しく送信されました#ct%1$s #tc%2$s';
$_LANGMAIL['Your message has been correctly sent'] = 'メッセージは正しく送信されました。';
$_LANGMAIL['New credit slip regarding your order'] = 'Guiding You - ご予約の伝票です';
$_LANGMAIL['The virtual product that you bought is available for download'] = '';
$_LANGMAIL['Your new password'] = '';
$_LANGMAIL['Fwd: Customer message'] = '';
$_LANGMAIL['Your guest account has been transformed into a customer account'] = '';
$_LANGMAIL['Package in transit'] = '';
$_LANGMAIL['Log: You have a new alert from your shop'] = '';
$_LANGMAIL['Your order has been changed'] = '';
$_LANGMAIL['Order confirmation'] = 'Guiding You - ご予約の確認';
$_LANGMAIL['Message from a customer'] = 'Guiding You - お客様メッセージ';
$_LANGMAIL['New message regarding your order'] = 'Guiding You - からのメッセージ';
$_LANGMAIL['Your order return status has changed'] = '';
$_LANGMAIL['Password query confirmation'] = '';
$_LANGMAIL['An answer to your message is available #ct%1$s #tc%2$s'] = '';
$_LANGMAIL['New voucher for your order #%s'] = '';
$_LANGMAIL['New voucher for your order %s'] = '';
$_LANGMAIL['Newsletter confirmation'] = '';
$_LANGMAIL['Email verification'] = '';
$_LANGMAIL['Newsletter voucher'] = '';
$_LANGMAIL['Your wishlist\\\'s link'] = '';
$_LANGMAIL['Message from %1$s %2$s'] = 'Guiding You - 欲しいものリスト';
$_LANGMAIL['%1$s sent you a link to %2$s'] = '';

?>